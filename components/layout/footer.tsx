...
const Footer: React.FC = () => {
  return (
    <footer className="bg-primary text-text-inverse py-8">
          ...
          <div className="flex space-x-4">
            {siteData.socialMedia.map((platform) => (
              <Link key={platform.name} href={platform.href} className="text-text-inverse hover:text-secondary transition-colors duration-200" aria-label={`Follow us on ${platform.name}`}>
                ...
                <svg className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" dangerouslySetInnerHTML={{ __html: socialIcons[platform.name] || '' }}>
                </svg>
              </Link>
            ))}
          </div>
